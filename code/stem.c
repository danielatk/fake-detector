...
static void stemfile(FILE * review, char* nome_review) //muito ineficiente mas foi o algoritmo que achei na internet e não deu para modificar completamente
   ...
   {  int ch = getc(review);
      ...
      {  int i = 0;
         ...
         {  if (i == i_max) increase_s();
            ...
            s[i] = ch; i++;
            ch = getc(review);
            if (!LETTER(ch)) { ungetc(ch,review); break; }
         }
         s[stem(s,0,i-1)+1] = 0;
